...
class WTDouble:
    ...
    def __update_configs(self, equipment_id: str, channel_name: str, sr: int):
        """
        根据设备和通道更新参数
    ...
    def rt_features(self, raw: np.ndarray, freq_rpm_high: np.ndarray, freq_rpm_low: np.ndarray, datetime: str,
                    equipment_id: str, channel_name: str, sr: int):
        """
        - 针对每3分钟数据提取统计特征;
    ...
    def diagnosis_report(self, datetime: str, equipment_id: str, channel_name: str, sr:int):
        """
        针对用于生成监测报告的3分钟数据，进行：
        ...
        plot_tools = Plot(self.save_dir, raw, self.wave_bp_low, self.wave_bp_high, self.sr, self.position,
                          self.position_mapping, self.plot_freq_upper_limit, self.channel_name, defect_matrices,
                          features["std"],
    ...
    def __rotational_frequency(self, rpm_high: np.ndarray, rpm_low: np.ndarray):
        """
        转频均值
        :param rpm_high: 传感器直接读出的高速端转频数组
        :param rpm_low: 传感器直接读出的低速端转频数组
        :return: float, 高速或低速端转频均值
        """
        if len(rpm_high):
            ratio = self.ratio_h
            freq_rpm = rpm_high / self.rpm_pulse / ratio
        else:
            ratio = self.ratio_l
            freq_rpm = rpm_low / self.rpm_pulse * ratio
    ...
    def __defect_diagnose(self, defect_matrices: dict):
        """
        根据缺陷矩阵，诊断缺陷类型
        :param defect_matrices: 缺陷频率的谐波矩阵
        :return: 字典：对应特征值的缺陷类型和严重等级
        """
        dg_tools = Diagnosis(self.surface_defect_dscrp, self.defect_snr_minor, self.defect_snr_obv,
                             self.defect_snr_vital,
